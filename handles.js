...
function getRedHandle(original, image_name) {
  ...
  const RED = [139, 29, 45];
  image = segmenter.getLargestByColor(image, RED, 80, image_name); // get binary image with BLACK whenever the is RED and the rest is white
...
function getBlackHandles(original, image_name) {
  ...
  image = segmenter.getLargestByColor(image, BLACK, 110, image_name); // get binary image with BLACK whenever the is RED and the rest is white
